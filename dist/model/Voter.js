    ...
    try {
        ...
        if (this.isModified('voterKey') || (this.voterKey && !this.voterKeyHash)) {
            ...
            if (!encryptionKey || encryptionKey === 'default-key-change-in-production') {
                console.warn('Warning: Using default encryption key. Set VOTER_KEY_ENCRYPTION_KEY in production!');
            }
            // Use the key (default or environment)
            const keyToUse = encryptionKey || 'default-key-change-in-production';
