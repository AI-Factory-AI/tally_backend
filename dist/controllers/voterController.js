...
exports.sendEmailsToPendingVoters = exports.exportVotersForDeployment = exports.verifyVoter = exports.deleteVoter = exports.updateVoterStatus = exports.getVoterStats = exports.getElectionVoters = exports.bulkImportVoters = exports.addVoter = void 0;
const Voter_1 = __importDefault(require("../model/Voter"));
const Election_1 = __importDefault(require("../model/Election"));
const express_validator_1 = require("express-validator");
const notificationMiddleware_1 = require("../middleware/notificationMiddleware");
...
const addVoter = async (req, res) => {
    try {
        ...
        let generatedVoterKey;
        let generatedVerificationToken;
        try {
            generatedVoterKey = Voter_1.default.generateVoterKey();
            generatedVerificationToken = Voter_1.default.generateVerificationToken();
        ...
        const voterKeyHash = crypto_1.default.createHash('sha256').update(generatedVoterKey).digest('hex');
        // Create voter
        const voter = new Voter_1.default({
            electionId,
        ...
        try {
            ...
            const mailInfo = await emailService_1.default.sendVoterCredentialsEmail({
                to: voter.email,
            ...
            req._emailPreviewUrl = nodemailer_1.default.getTestMessageUrl(mailInfo) || undefined;
        }
        catch (emailError) {
        ...
        try {
            await (0, notificationMiddleware_1.sendNotification)({
        ...
        res.status(201).json({
            ...
            voter: voterResponse,
            emailPreviewUrl: req._emailPreviewUrl
        });
    }
    catch (error) {
    ...
    try {
        ...
        for (let i = 0; i < voters.length; i++) {
            ...
            try {
                ...
                // Generate secure voter key
                const generatedVoterKey = Voter_1.default.generateVoterKey();
                const generatedVerificationToken = Voter_1.default.generateVerificationToken();
                const normalizedKey = Voter_1.default.generateVoterKey();
        ...
        if (votersToAdd.length > 0) {
            const insertedVoters = await Voter_1.default.insertMany(votersToAdd);
            ...
            for (const voter of insertedVoters) {
                try {
                    ...
                    const rawVoterKey = voter.decryptVoterKey ? voter.decryptVoterKey() : voter.voterKey;
                    await emailService_1.default.sendVoterCredentialsEmail({
                        to: voter.email,
                        voterName: voter.name,
                        electionTitle: election.title,
                        voterId: voter.uniqueId,
...
exports.bulkImportVoters = bulkImportVoters;
...
exports.getElectionVoters = getElectionVoters;
...
const getVoterStats = async (req, res) => {
    try {
        ...
        const stats = await Voter_1.default.aggregate([
        ...
        const totalVoters = await Voter_1.default.countDocuments({ electionId });
        const verifiedVoters = await Voter_1.default.countDocuments({
            electionId,
...
exports.getVoterStats = getVoterStats;
// Update voter status
const updateVoterStatus = async (req, res) => {
    try {
        ...
        // Update voter status
        const voter = await Voter_1.default.findOneAndUpdate({ _id: voterId, electionId }, {
            status,
            ...(status === 'VERIFIED' && { verifiedAt: new Date() }),
            lastActivity: new Date()
        }, { new: true }).select('-voterKey -verificationToken');
        if (!voter) {
            return res.status(404).json({ message: 'Voter not found' });
        }
        // Send email notification to voter about status change
        try {
            await emailService_1.default.sendVoterStatusUpdateEmail(voter.email, voter.name, election.title, status);
...
exports.updateVoterStatus = updateVoterStatus;
// Delete a voter
const deleteVoter = async (req, res) => {
    try {
        ...
        const voterToDelete = await Voter_1.default.findOne({ _id: voterId, electionId })
            .select('name email uniqueId');
        if (!voterToDelete) {
            return res.status(404).json({ message: 'Voter not found' });
        }
        // Delete voter
        await Voter_1.default.findOneAndDelete({ _id: voterId, electionId });
...
const exportVotersForDeployment = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Authentication required' });
        }
...
exports.exportVotersForDeployment = exportVotersForDeployment;
// Send emails to all pending voters and activate them
const sendEmailsToPendingVoters = async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Authentication required' });
        }
        ...
        const pendingVoters = await Voter_1.default.find({
            electionId,
            status: 'PENDING'
        }).select('+voterKey');
...
exports.sendEmailsToPendingVoters = sendEmailsToPendingVoters;
