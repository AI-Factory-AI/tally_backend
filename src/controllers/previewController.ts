...
export const generatePreviewUrl = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
...
export const getElectionPreview = async (req: Request, res: Response) => {
  try {
    ...
    // Get voter count
    const voterCount = await Voter.countDocuments({ electionId });
...
export const getElectionPreviewMobile = async (req: Request, res: Response) => {
  try {
    const { electionId } = req.params;
...
export const validatePreviewAccess = async (req: Request, res: Response) => {
  try {
    const { electionId, previewToken } = req.params;
...
export const getPreviewStatistics = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Authentication required' });
    }
    ...
    const hasBallot = !!(await Ballot.findOne({ electionId, isActive: true }));
    const voterCount = await Voter.countDocuments({ electionId });
    const verifiedVoterCount = await Voter.countDocuments({ 
      electionId, 
    ...
    if (election.title && election.description) readinessScore += 20;
    if (election.startTime && election.endTime) readinessScore += 20;
    if (election.maxVotersCount > 0) readinessScore += 20;
    if (hasBallot) readinessScore += 20;
    if (voterCount > 0) readinessScore += 20;
    ...
    res.status(200).json({
      ...
      ballot: {
        hasBallot,
        questionCount: hasBallot ? (await Ballot.findOne({ electionId, isActive: true }))?.questions?.length || 0 : 0
      },
      voters: {
        total: voterCount,
...
export const generateLiveUrl = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Authentication required' });
    }
